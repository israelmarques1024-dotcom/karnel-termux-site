      ...
      <div className="max-w-4xl mx-auto">
        <AnimatedSection>
          <h1 className="text-4xl font-bold font-mono mb-4">
            Stack Linux Completa
          </h1>
          <p className="text-lg text-muted-foreground mb-8">
            Karnel transforms your Android into a complete Linux environment
            with languages, databases, and development tools.
          </p>
        </AnimatedSection>
          ...
          {stacks.slice(0, 7).map((item, i) => (
            <AnimatedSection key={i} delay={300 + i * 60}>
              <div className="card-hover bg-card border border-border rounded-lg p-5">
                <h3 className="font-bold font-mono text-accent mb-1">
                  {item.name}
                </h3>
                ...
                <CodeBlock
                  code={`karnel install lang --${item.pkg}`}
                  language="bash"
                />
              </div>
            </AnimatedSection>
          ...
          {stacks.slice(7).map((item, i) => (
            <AnimatedSection key={i} delay={800 + i * 80}>
              <div className="card-hover bg-card border border-border rounded-lg p-5">
                ...
                <CodeBlock
                  code={`karnel install db --${item.pkg}`}
                  language="bash"
                />
              </div>
            </AnimatedSection>
        ...
        <AnimatedSection delay={1100}>
          ...
          <div className="card-hover bg-card border border-border rounded-lg p-6">
            <p className="text-muted-foreground mb-4">
              Com o Karnel, seu dispositivo Android vira um ambiente de
              desenvolvimento completo. Você pode:
            </p>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex gap-2">
                <span className="text-accent">•</span> Rodar servidores Node.js,
                Python e Go
              </li>
              <li className="flex gap-2">
                <span className="text-accent">•</span> Conectar em bancos
                PostgreSQL e MongoDB
              </li>
              <li className="flex gap-2">
                <span className="text-accent">•</span> Compilar código Rust e
                C/C++
              </li>
              <li className="flex gap-2">
                <span className="text-accent">•</span> Fazer deploy direto do
                seu celular
              </li>
              <li className="flex gap-2">
                <span className="text-accent">•</span> Desenvolver apps
                completos sem precisar de PC
              </li>
            </ul>
          </div>
        </AnimatedSection>
